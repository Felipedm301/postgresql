--Consulta com filtro acrescentado
                ...
                (--Com filtro acrescentado
					SELECT CASE
						...
						WHERE requisicao.id = requisicao_servico_id
							...
							FROM atende,historico_solicitacao hs 
							WHERE  requisicao.id = requisicao_servico_id) THEN 'Encaminhado'
						WHEN (SELECT Count(DISTINCT( solicitacao_id )) 
						FROM   atende.historico_solicitacao hs 
						WHERE  requisicao.id = requisicao_servico_id 
							AND situacao_solicitacao_id IN( 4, 8, 5, 9, 3 )) > 0 THEN 'Parcialmente encaminhado'
						--Novo
						WHEN (SELECT Count(DISTINCT( solicitacao_id )) 
						FROM   atende.historico_solicitacao hs 
						WHERE  requisicao.id = requisicao_servico_id 
							AND situacao_solicitacao_id IN(5)) > 0 THEN 'Cancelado'
